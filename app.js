...
function renderCart(){
 ...
 box.innerHTML=cart.map(row=>{
   ...
   return `<div class="cart-row">
     ...
     <div><h4>${esc(p.name)}</h4><small>${money(unitPrice)} DH / unité · ${p.qty} unités / boîte</small>${promoNote}
...
function renderCarouselLevelDisplay(){
 ...
 box.innerHTML=`<div class="levels-display-top"><div><small>خصومات الكمية</small></div><span>${discountCount} خصومات</span></div><div class="levels-display-track">${tiers.map((tier,index)=>{const quantity=tier.maxQty==null?`${tier.minQty}+`:`${tier.maxQty}`,total=tier.maxQty==null?"مفتوح":`${tier.maxQty} = ${money(tier.maxQty*tier.price)} DH`;return `<div class="levels-display-card" style="--level-delay:${index*45}ms"><b>${quantity} قطعة</b><span>${money(tier.price)} DH / قطعة</span><strong>${total}</strong></div>`}).join("")}</div>${promo}<div class="levels-display-hint">اضغط على السهم للانتقال إلى منتوج آخر · الخصومات والأسعار تتحدث تلقائيًا</div>`;
...
function openProductFocus(id){
 ...
 $("focusPrice").textContent=`${money(priceTiersFor(p)[0].price)} DH / unité`;
 ...
 $("focusBackPrice").textContent=`${money(priceTiersFor(p)[0].price)} DH`;
...
function priceTierSlider(p){
 ...
 const level=tiers[0],range=level.maxQty==null?`${level.minQty}+`:`${level.minQty}–${level.maxQty}`,total=level.maxQty==null?"مفتوح":`${level.maxQty} قطعة = ${money(level.maxQty*level.price)} DH`;
 return `<div class="price-tier-slider" data-tier-slider="${esc(p.id)}" data-tier-count="${tiers.length}">
   ...
   <div class="tier-info-card"><div><small>الفترة المختارة</small><strong class="tier-range">${range} قطعة</strong></div><div><small>ثمن القطعة</small><strong class="tier-unit-price">${money(level.price)} DH</strong></div><div><small>الإجمالي عند الحد</small><strong class="tier-total">${total}</strong></div></div>
 </div>`;
}
function updateTierSlider(slider,index){
 ...
 slider.querySelector(".tier-unit-price").textContent=`${money(tier.price)} DH`;
 slider.querySelector(".tier-total").textContent=tier.maxQty==null?"مفتوح":`${tier.maxQty} قطعة = ${money(tier.maxQty*tier.price)} DH`;
  ...
  <div class="flip-card-inner">
   <div class="flip-face flip-front">
    ...
    <div class="card-body">
     ...
     <div class="price">${money(priceTiersFor(p)[0].price)} <small>DH / unité</small></div>
   ...
   <div class="flip-face flip-back">
    ...
    <div class="flip-specs">
      <div><span>PRIX / UNITÉ</span><strong>${money(priceTiersFor(p)[0].price)} DH</strong></div>
...
async function createOrderImage(order){
 try{
  ...
  const installmentSummaryHtml=isCodTerm?"":`<div style="display:flex;justify-content:space-between;gap:10px;margin-top:7px;color:#667085;font-size:12px"><span>مجموع الأقساط</span><span>${money(paid)} DH</span></div>`;
...
async function sharePaymentSummaryWithCustomer(){
 ...
 const history=getPaymentHistory(order),changes=getPriceChangeHistory(order);
 if(!history.length&&!changes.length){toast("ما كاين لا قسط لا تغيير ثمن باش تتوجد الصورة");return}
 ...
 return sharePaymentOperationImage({orderId:order.id,amount,beforeDue:before,afterDue:before-amount,totalPaidAfter:paymentTotal(order),priceChanges:changes,priceOnly:!amount,operationProvided:true,operationChanges:changes});
}
async function sharePaymentOperationImage(context={}){
 ...
 if(context.operationProvided===true&&!((Number.isFinite(amount)&&amount>0)||(Array.isArray(context.operationChanges)&&context.operationChanges.length))){toast("ما كاين حتى قسط أو تغيير ثمن باش تتوجد الصورة");return}
 ...
 const accepted=Math.min(amount,before),afterDue=Number.isFinite(Number(context.afterDue))?Math.max(0,Number(context.afterDue)):Math.max(0,before-accepted),totalPaidForImage=Number.isFinite(Number(context.totalPaidAfter))?Number(context.totalPaidAfter):paymentTotal(order)+accepted,adjustedTotal=Number(order.total)||0,hasPaymentOperation=amount>0,hasPriceOperation=priceChanges.length>0,operationTitle=hasPaymentOperation&&hasPriceOperation?"ملخص القسط وتغيير الثمن":hasPaymentOperation?"ملخص عملية الأداء":"ملخص تغيير الأثمنة",currentAmountLabel=hasPaymentOperation?"القسط الحالي":"تغيير الثمن فقط",now=new Date(),date=now.toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}),time=now.toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"});
 const root=document.createElement("div");root.dir="rtl";root.style.cssText="position:fixed;left:-10000px;top:0;width:760px;min-height:400px;background:#f5f7fb;color:#172033;padding:28px;font-family:Arial,'Noto Naskh Arabic',sans-serif;box-sizing:border-box;z-index:999999;visibility:visible;opacity:1;direction:rtl;text-align:right";
...
function savePaymentForm(e){
 ...
 const orderId=order.id,afterDue=Math.max(0,Number(order.total||0)-paymentTotal(order)),totalPaidAfter=paymentTotal(order),imageContext={orderId,amount:hasAmount?amount:0,beforeDue:remaining,afterDue,totalPaidAfter,priceChanges:getPriceChangeHistory(order),priceOnly:onlyPrice||!hasAmount,operationProvided:true,operationChanges:changedPriceChanges};
...
function renderDashboard(){
 ...
 setText("dashboardOrdersCount",String(selectedOrders.length));setText("dashboardSalesTotal",`${money(sales)} DH`);setText("dashboardTopProduct",topProducts[0]?.name||"—");setText("dashboardPeakHour",peakText);
 setText("dashboardCostTotal",`${money(cost)} DH`);setText("dashboardProfitTotal",`${money(profit)} DH`);setText("dashboardPaidTotal",`${money(paid)} DH`);setText("dashboardDueTotal",`${money(due)} DH`);setText("dashboardUnitsTotal",`${unitsSold} قطعة`);setText("dashboardClientsTotal",String(clientMap.size));setText("dashboardCitiesTotal",String(cityMap.size));setText("dashboardMarginTotal",`${margin.toFixed(1)}%`);setText("dashboardPaidOrders",String(paidOrders));setText("dashboardDueOrders",String(dueOrders));setText("dashboardFreeUnits",`${freeUnits} قطعة`);
 const renderRankRows=(items,type)=>items.length?items.map((item,index)=>{const main=type==="product"?`${item.units} قطعة`:type==="client"?`${item.orders} طلب`:`${item.orders} طلب`;const name=type==="city"?item.city:item.name;return `<div class="dashboard-rank-row"><span class="dashboard-rank-index">${index+1}</span><div><b>${esc(name)}</b><small>${main} · مخلص ${money(item.paid)} DH · باقي ${money(item.due)} DH</small></div><strong>${money(item.sales)} DH</strong></div>`}).join(""):"<div class=\"dashboard-table-empty\">ما كايناش معطيات فهاد الشهر.</div>";
...
function updateViewerBoxTotal(p){
 ...
 $("viewerPrice2").textContent=`${money(unitPrice)} DH`;
}
...
function view(id){
 ...
 $("viewerPrice2").textContent=`${money(unitPriceForQuantity(p,Number(p.qty)||1))} DH`;
