...
async function createOrderImage(order){
 try{
  ...
  const d=new Date(order.date),date=d.toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}),time=d.toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"}),changes=getPriceChangeHistory(order),payments=getPaymentHistory(order),paid=paymentTotal(order),total=Number(order.total)||0,baseTotal=Number(order.baseTotal),hasChanges=changes.length>0,priceAdjustmentTotal=changes.reduce((sum,change)=>sum+priceChangeAdjustment(order,change),0);
   ...
   const returnsSectionHtml=hasReturns?`<div style="margin-top:17px;padding:14px;border:2px solid #17824b;border-radius:14px;background:#ecfdf3"><div style="display:flex;justify-content:space-between;gap:10px;color:#087443;font-size:17px;font-weight:900"><span>الإرجاعات / Retours</span><strong>−${money(returnsTotal)} درهم</strong></div>${returns.map((item,index)=>`<div style="display:flex;justify-content:space-between;gap:10px;margin-top:8px;padding:9px 10px;border:1px solid #9bd5b2;border-radius:10px;background:#fff"><span><b style="color:#087443">${esc(item.productName||"منتوج")}${item.productCode?` · ${esc(item.productCode)}`:""}</b><small style="display:block;color:#157347;margin-top:3px;direction:ltr;text-align:left">الإرجاع: ${Number(item.quantity)||0} قطعة × ${money(item.unitPrice||0)} درهم = ${money(item.amount||0)} درهم · ${formatPaymentDate(item.date)}</small></span><strong style="color:#087443">−${money(item.amount)} درهم</strong></div>`).join("")}</div>`:"";
...
async function sharePaymentOperationImage(context={}){
 ...
 if(context.operationProvided===true&&!((Number.isFinite(amount)&&amount>0)||(Array.isArray(context.operationChanges)&&context.operationChanges.length)||(context.operationDiscountProvided===true)||contextDiscount>0||(context.operationReturnProvided===true)||returnHistory.length)){toast("ما كاين حتى قسط لا تخفيض لا إرجاع لا تغيير ثمن باش تتوجد الصورة");return}
 ...
 if(amount>before+0.000001){alert(`المبلغ أكبر من الباقي: ${money(before)} درهم`);return}
 const accepted=Math.min(amount,before),afterDue=Number.isFinite(Number(context.afterDue))?Math.max(0,Number(context.afterDue)):Math.max(0,before-accepted),totalPaidForImage=Number.isFinite(Number(context.totalPaidAfter))?Number(context.totalPaidAfter):paymentTotal(order)+accepted,adjustedTotal=Number(order.total)||0,returnTotalForImage=returnHistory.reduce((sum,item)=>sum+(Number(item.amount)||0),0),hasPaymentOperation=amount>0,hasPriceOperation=priceChanges.length>0,hasReturnOperation=returnHistory.length>0,operationTitle=hasPaymentOperation&&hasPriceOperation?"ملخص القسط وتغيير الثمن":hasPaymentOperation&&hasReturnOperation?"ملخص القسط والإرجاع":hasPaymentOperation?"ملخص عملية الأداء":hasPriceOperation&&hasReturnOperation?"ملخص تغيير الثمن والإرجاع":hasPriceOperation?"ملخص تغيير الأثمنة":hasReturnOperation?"ملخص الإرجاعات":"ملخص التخفيض",currentAmountLabel=hasPaymentOperation?"القسط الحالي":hasPriceOperation?"تغيير الثمن فقط":hasReturnOperation?"الإرجاعات / Retours":"Remise / تخفيض",now=new Date(),date=now.toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric"}),time=now.toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"});
 ...
 const discountTotalForImage=discountHistory.reduce((sum,item)=>sum+(Number(item.amount)||0),0),hasDiscountOperation=discountHistory.length>0||contextDiscount>0;
 const priceChangesTotal=priceChanges.reduce((sum,change)=>sum+Math.abs(priceChangeAdjustment(order,change)),0),paymentHistoryHtml=paymentHistory.length?paymentHistory.map((payment,index)=>`<div style="display:flex;justify-content:space-between;gap:8px;padding:9px 10px;border:1px solid #e4e8ef;border-radius:10px;background:#f7f9fc;margin-top:7px"><span><b>قسط رقم ${index+1}</b><small style="display:block;color:#667085;margin-top:3px">${formatPaymentDate(payment.date)}</small></span><strong style="color:#173f78">${money(payment.amount)} درهم</strong></div>`).join(""):`<div style="padding:10px;color:#98a2b3;text-align:center">مازال ما تسجل حتى قسط</div>`,changesHtml=priceChanges.length?priceChanges.map(change=>{const difference=priceChangeUnitDifference(change),quantity=priceChangeQuantity(order,change),adjustment=priceChangeAdjustment(order,change);return `<div style="display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center;padding:13px 14px;border:1px solid #ead9ad;border-radius:12px;background:#fffaf0;margin-top:8px"><div><b style="display:block;font-size:15px;color:#172033">${esc(change.productName)}${change.productCode?` · ${change.productCode}`:""}</b><span style="display:block;margin-top:5px;color:#667085;font-size:12px">الثمن القديم: ${money(change.oldPrice)} درهم · الثمن الجديد: ${money(change.newPrice)} درهم · الكمية: ${quantity}</span><span style="display:block;margin-top:4px;color:${adjustment>0?"#b42318":"#087443"};font-size:12px;font-weight:800">أثر التغيير على الكوموند: ${adjustment>0?"+":"−"}${money(Math.abs(adjustment))} درهم</span></div><strong style="font-size:16px;color:${adjustment>0?"#b42318":"#087443"};direction:ltr">${adjustment>0?"+":"−"}${money(Math.abs(adjustment))} درهم</strong></div>`}).join(""):"<div style=\"padding:13px;border:1px dashed #d8dee8;border-radius:12px;background:#fff;color:#98a2b3;text-align:center;font-size:12px\">ما تسجل حتى تغيير فالثمن مع هاد القسط.</div>";
 const paymentSectionHtml=hasPaymentOperation?`<div style="font-size:17px;font-weight:900;color:#173f78;margin:18px 0 8px">سجل الأقساط السابقة</div><div style="padding:10px;border:1px solid #e4e8ef;border-radius:12px;background:#f7f9fc">${paymentHistoryHtml}</div>`:"";
 const changesSectionHtml=hasPriceOperation?`<div style="font-size:17px;font-weight:900;color:#173f78;margin:18px 0 8px">تغيير أثمنة المنتوجات <span style="font-size:13px;color:#667085">(${money(priceChangesTotal)} درهم)</span></div>${changesHtml}`:"";
 const returnsSectionHtml=hasReturnOperation?`<div style="font-size:17px;font-weight:900;color:#087443;margin:18px 0 8px">الإرجاعات / Retours <span style="font-size:13px;color:#157347">(−${money(returnTotalForImage)} درهم)</span></div>${returnHistory.map(item=>`<div style="display:flex;justify-content:space-between;gap:8px;padding:9px 10px;border:1px solid #9bd5b2;border-radius:10px;background:#ecfdf3;margin-top:7px"><span><b style="color:#087443">${esc(item.productName||"منتوج")}${item.productCode?` · ${esc(item.productCode)}`:""}</b><small style="display:block;color:#157347;margin-top:3px;direction:ltr;text-align:left">الإرجاع: ${Number(item.quantity)||0} قطعة × ${money(item.unitPrice||0)} درهم = ${money(item.amount||0)} درهم · ${formatPaymentDate(item.date)}</small></span><strong style="color:#087443">−${money(item.amount)} درهم</strong></div>`).join("")}`:"";
 const discountsSectionHtml=hasDiscountOperation?`<div style="font-size:17px;font-weight:900;color:#087443;margin:18px 0 8px">Remise / تخفيضات <span style="font-size:13px;color:#157347">(${money(discountTotalForImage)} درهم)</span></div>${discountHistory.map((item,index)=>`<div style="display:flex;justify-content:space-between;gap:8px;padding:9px 10px;border:1px solid #9bd5b2;border-radius:10px;background:#ecfdf3;margin-top:7px"><span><b style="color:#087443">تخفيض رقم ${index+1}</b><small style="display:block;color:#157347;margin-top:3px">${formatPaymentDate(item.date)}</small></span><strong style="color:#087443">−${money(item.amount)} درهم</strong></div>`).join("")}`:"";
...
function assistantProductAnswer(product,text){
  ...
  const lines=[`المنتوج هو: ${product.name||"بلا اسم"}.`,productCode(product)?`الكود ديالو: ${productCode(product)}.`:"",`الثمن دابا: ${money(currentPrice)} درهم للقطعة${requestedQty?` فكمية ${requestedQty} قطعة`:""}.`,product.qty?`العلبة فيها ${Number(product.qty)||0} قطعة.`:"",isAvailable(product)?"المنتوج متوفر دابا.":"هاد المنتوج ما متوفرش دابا."];
  if(tiers.length){lines.push("الأثمنة حسب الكمية:");lines.push(tiers.map(t=>`${t.maxQty==null?`${t.minQty} وفوق`:`من ${t.minQty} حتى ${t.maxQty} قطعة`}: ${money(t.price)} درهم للقطعة`).join("\n"));}
...
function assistantAnswer(text){
  ...
  if(assistantHas(q,["السله","السلة","panier","cart","شنو شريت","المشتريات"])){
    ...
    return c.count?`السلة فيها ${c.count} منتوج، بمجموع ${c.units} قطعة. المجموع المحسوب هو ${money(c.total)} درهم. حل السلة من زر 🛒 باش تشوف التفاصيل وتكمل الطلب.`:"السلة خاوية دابا. اختار شي منتوج وضغط على زر الإضافة للسلة.";
  }
  ...
  if(assistantHas(q,["طلبيه","الطلبيات","طلبية","commandes","orders","كوموند"])){
    ...
    return `مسجلين دابا ${o.count} طلبية. مجموع الطلبيات هو ${money(o.total)} درهم، والمخلص ${money(o.paid)} درهم، والباقي ${money(o.due)} درهم. دخل من القائمة على الطلبيات باش تشوف التفاصيل.`;
  }
  if(assistantHas(q,["قسط","الاقساط","الأقساط","دين","الديون","الباقي","استخلاص","مستخلاص","المستخلاصات"])){
    ...
    return `تتبع الأقساط والمستخلاصات كاين فالقائمة. دابا مجموع الأقساط المسجلين هو ${payments} عملية، والباقي الإجمالي فالكوموندات هو ${money(o.due)} درهم. من تتبع المستخلاصات تقدر تشوف عند الاستلام، الأقساط، والإجمالي وتقدر تصفر الدورة مع بقاء السجل القديم.`;
  }
  ...
  if(assistantHas(q,["احصائيات","الإحصائيات","dashboard","لوحه التحكم","المبيعات","أوقات الذروة","الربح","الشراء","المدن"]))return "الإحصائيات العامة ولات كتوريك عدد الطلبيات، مجموع المبيعات، ثمن الشراء أو التكلفة التقريبية، الربح وهامش الربح، الخلاص المحصل والباقي، القطع المباعة والمجانية، الكليان والمدن الأكثر بيعاً، أفضل المنتوجات، والكوموندات المخلصة أو اللي باقي فيها؛ وتقدر تختار الشهر وتشوف تطور المبيعات والخلاص والباقي عبر الأشهر.";
...
function assistantFormatMessage(text){return esc(text).replace(/(\d[\d\s., ]*?)\s*(?:درهم|Dh)\b/gi,(match,raw)=>{const value=Number(String(raw).replace(/[\s ]/g,"").replace(",","."));return Number.isFinite(value)?`<b class="assistant-money">${value.toFixed(2)} درهم</b>`:match}).replace(/\n/g,"<br>")}
...
function aiAgentMoney(v){return `${(Number(v)||0).toFixed(2)} درهم`}
